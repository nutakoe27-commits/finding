...
# Источники ежедневного прогона, в порядке сбора. Порядок содержательный:
# fns_open_data первым, потому что создаёт универсум компаний — без него
# краулеры не смогут свести найденные названия к ИНН, и вся добыча уйдёт
# в карантин с «not_found» вместо ожиданий. registry последним: он обогащает
# компании, которые добавили остальные.
COLLECT_ORDER = (
    "fns_open_data",
    "manual",
    "events_archive",
...
# Источники, которые в ежедневный прогон НЕ входят и запускаются отдельно.
#
# venue_pages обходит десятки чужих сайтов с паузами между запросами: это
# десятки минут, и делать это каждое утро незачем — страницы «прошедшие
# мероприятия» обновляются раз в сезон, не раз в сутки. Плюс один медленный
# сайт не должен задерживать выдачу утреннего списка.
#
#     gtm collect venue_pages          # раз в квартал, руками или отдельным cron
PERIODIC_SOURCES = ("venue_pages",)
